...
class PaymentMethodCreateView(PermissionMixin, CreateViewMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        success_message = f"Éxito al crear el Método de Pago {payment_method.description}."
...
class PaymentMethodUpdateView(PermissionMixin, UpdateViewMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        payment_method = self.object
        success_message = f"Éxito al actualizar el Método de Pago {payment_method.description}."
        messages.success(self.request, success_message)
        Notification.objects.create(message=success_message)
        return response
